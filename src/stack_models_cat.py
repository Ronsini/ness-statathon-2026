"""
NESS Statathon 2026 — attempt-21: stacking with XGB + LGB + CatBoost (9 meta features).

Extends attempt-20 by adding CatBoost OOF probabilities as a third base model.
No retraining of any base model.

Meta features (9 per row):
  xgb_prob_0, xgb_prob_1, xgb_prob_2,
  lgb_prob_0, lgb_prob_1, lgb_prob_2,
  cat_prob_0, cat_prob_1, cat_prob_2

Outputs:
  output/stack_cat_results.txt        per-config results + best summary
  output/submission_stack_cat.csv     Kaggle submission (id, Predicted)
  output/stack_cat_oof_probs.npy      best-config OOF meta-probs  (n_train, 3)
  output/stack_cat_test_probs.npy     best-config test meta-probs (n_test, 3)

Usage:
  python src/stack_models_cat.py
"""
...
BLEND_OOF_BASELINE = 0.73682  # attempt-19
STACK_OOF_BASELINE = 0.73788  # attempt-20 (XGB+LGB stack)
XGB_PUBLIC_BEST    = 0.76038  # attempt-20 public
...
def main():
    ...
    meta_train = np.hstack([xgb_oof,  lgb_oof,  cat_oof])   # (n_train, 9)
    meta_test  = np.hstack([xgb_test, lgb_test, cat_test])  # (n_test, 9)

    print(f"Meta train: {meta_train.shape}  Meta test: {meta_test.shape}", flush=True)
    print(
        f"Baselines:  XGB={XGB_OOF_BASELINE}  Blend={BLEND_OOF_BASELINE}  "
        f"Stack(XGB+LGB)={STACK_OOF_BASELINE}  Best public={XGB_PUBLIC_BEST}",
        flush=True,
    )
    ...
    if beats_stack:
        verdict = f"BEATS attempt-20 XGB+LGB stack OOF by {best_tuned - STACK_OOF_BASELINE:+.5f}"
    elif beats_xgb:
        verdict = (
            f"Beats XGB-only OOF by {best_tuned - XGB_OOF_BASELINE:+.5f} "
            f"but not attempt-20 stack ({best_tuned - STACK_OOF_BASELINE:+.5f})"
        )
    else:
        verdict = f"Does NOT beat XGB OOF ({best_tuned - XGB_OOF_BASELINE:+.5f})"
    ...
    summary_lines = [
        ...
        f"  attempt-20 public                  : {XGB_PUBLIC_BEST}",
        f"",
        f"Verdict: {verdict}",
        f"",
    ...
    results_path = OUTPUT_DIR / "stack_cat_results.txt"
    ...
    sub_path = OUTPUT_DIR / "submission_stack_cat.csv"
    sub.to_csv(sub_path, index=False)
    print(f"Saved submission to {sub_path}", flush=True)

    np.save(OUTPUT_DIR / "stack_cat_oof_probs.npy",  best_oof)
    np.save(OUTPUT_DIR / "stack_cat_test_probs.npy", best_test)
    print("Saved stack_cat probability arrays to output/", flush=True)
