"""
NESS Statathon 2026 — Save LightGBM (attempt-8) probability arrays.

Replicates the exact attempt-8 pipeline (LGB params, preprocessing, OOF encoding,
class weights, 5-fold CV) and saves OOF + test probabilities for blending/stacking.

Outputs:
  output/lgb_oof_probs.npy   shape (n_train, 3)
  output/lgb_test_probs.npy  shape (n_test, 3)
  output/y_train.npy         shape (n_train,)   — also written by train_model.py
  output/test_ids.npy        shape (n_test,)    — also written by train_model.py

Usage:
  python src/save_lgb_probs.py
"""
...
# Exact attempt-8 LightGBM params
LGB_PARAMS = {
...
def main():
    ...
    # Two-stage multiplier tuning (same as train_model.py)
    best_acc = raw_acc
    ...
    print(f"\nLGB 5-fold CV (mean): {np.mean(fold_scores):.5f}  std: {np.std(fold_scores):.5f}")
    ...
    np.save(OUTPUT_DIR / "lgb_oof_probs.npy", oof_preds)
    np.save(OUTPUT_DIR / "lgb_test_probs.npy", test_preds)
    np.save(OUTPUT_DIR / "y_train.npy", y)
    np.save(OUTPUT_DIR / "test_ids.npy", test_ids)
    log("Saved LGB probability arrays to output/", t0)
